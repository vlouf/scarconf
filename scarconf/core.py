...
class S3car:
    def __init__(
        ...
        self.tier2_radars = [1, 5, 6, 7, 14, 15, 16, 17, 29, 31, 32, 38, 42, 46, 49, 50, 55, 58]
        self.tier2_radars.extend([67, 69, 72, 74, 75, 77, 78, 79, 93, 94, 95, 96, 98, 107, 108, 109, 110])
    ...
    def get_kpi_pointing(self, elev: float, azimuth: float = None) -> int:
        ...
        ========
        kpi: int
            Green (1), Amber (2) or Red (3) alert.
        """
        if elev is None:
            angle = None
        else:
            if azimuth is None:
                azimuth = 0
            angle = np.max([np.abs(elev), np.abs(azimuth)])
    ...
    def get_kpi_reflectivity(self, tier: int, offset: float) -> int:
        ...
        if tier == 1:
            if np.abs(offset) < 1:
                return 1
            elif np.abs(offset) >= 1 and np.abs(offset) < 2:
                return 2
            else:
                return 3
        else:
            if np.abs(offset) < 3:
                return 1
            elif np.abs(offset) >= 3 and np.abs(offset) < 5:
                return 2
            else:
                return 3
    ...
    def calc_region(self):
        ...
        def lon2mx(lon):
            return eq_radius * math.radians(lon)

        def lat2my(lat):
            return eq_radius * math.log(math.tan(math.pi / 4 + math.radians(lat) / 2))
